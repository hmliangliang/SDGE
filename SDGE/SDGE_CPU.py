...
class Net():#定义图卷积神经网络结构
    def __init__(self,parameters,graph,r=2,X=None):
        ...
        self.r = r
        if X == None:
            self.X = torch.eye(self.graph.number_of_nodes())
        else:
            self.X = X
    ...
    def forward(self):
        ...
        D = torch.diag(torch.sum(A, dim=1)).sqrt().float()
        # 三隐含层连接权值
        H = self.X.float()
        # 三层图卷积操作
        H = D.mm(A.mm(D.mm(H.mm(self.W1))))  # 第1层卷积层
        model = DynamicReLU.DyReLUB(channels=H.shape[0], conv_type='1d')
        H = model(H.reshape(1,H.shape[0],H.shape[1]))[0]
        Normalization = nn.BatchNorm1d(H.shape[1],affine=True)
        H = Normalization(H)
        H = D.mm(A.mm(D.mm(H.mm(self.W2))))  # 第2层卷积层
        model = DynamicReLU.DyReLUB(channels=H.shape[0], conv_type='1d')
        H = model(H.reshape(1, H.shape[0], H.shape[1]))[0]
        Normalization = nn.BatchNorm1d(H.shape[1], affine=True)
        H = Normalization(H)
        H = D.mm(A.mm(D.mm(H.mm(self.W3))))  # 第3层卷积层
        model = DynamicReLU.DyReLUB(channels=H.shape[0], conv_type='1d')
        H = model(H.reshape(1, H.shape[0], H.shape[1]))[0]
        H = D.mm(A.mm(D.mm(H.mm(self.W4))))  # 输出层
        Normalization = nn.BatchNorm1d(H.shape[1], affine=True)
        H = Normalization(H)
        return H
...
class SDGE():
    def __init__(self,graph,X,parameters,num):
        ...
        self.graph = graph
        self.num = num
        #self.feature = torch.tensor(SpectralEmbedding(n_components=parameters.d).fit_transform(np.array(nx.adjacency_matrix(graph).todense())),dtype= torch.float32)
        if X == None:
            self.X = torch.eye(self.graph.number_of_nodes())
        else:
            self.X = X
    ...
    def noise(self, signal):  # 生成高斯分布噪声 https://blog.csdn.net/sinat_24259567/article/details/93889547
        SNR = 5
        noise = torch.randn(signal.shape[0], signal.shape[1])  # 产生N(0,1)噪声数据
    ...
    def LossFunction(self, Z):
        A = torch.tensor(np.array(nx.adjacency_matrix(self.graph).todense()))
        Xn = self.noise(Z)
        # 自监督学习损失
        ...
        del A
        # 图的正则化约束
        L = torch.tensor(nx.laplacian_matrix(self.graph).todense()).float()
    ...
    def fit(self):
        ...
        h1 = net1.forward()
        H.append(h1.detach().numpy())
        net2 = Net(self.parameters, self.graph, 2, self.X)
        h2 = net2.forward()
        H.append(h2.detach().numpy())
        net3 = Net(self.parameters, self.graph, 3, self.X)
        h3 = net3.forward()
        H.append(h3.detach().numpy())
        net4 = Net(self.parameters, self.graph, 4, self.X)
        h4 = net4.forward()
        H.append(h4.detach().numpy())
        w = self.weight(H)
        if self.parameters.cat == 'sum':  # 加权求和
            H = w[0] * h1 + w[1] * h2 + w[2] * h3 + w[3] * h4
        else:  # 拼接各个GCN的输出结果
            H = torch.tensor([])
            H = torch.cat((H, w[0] * h1), 1)
            H = torch.cat((H, w[1] * h2), 1)
            H = torch.cat((H, w[2] * h3), 1)
            H = torch.cat((H, w[3] * h4), 1)
        if X.shape[0] == X.shape[1]:
            if (X == torch.eye(X.shape[1])).any() == False:  # 判断是否是单位阵
               H = torch.cat((H, self.X), 1)
        else:
            H = torch.cat((H, self.X), 1)
        if self.parameters.fun == 'softmax':  # 采用端到端方式,直接获得社团划分结果
            net5 = NetF(H.shape[1],self.parameters.k)
        else:  # 使用k-means算法获得类簇的划分
            net5 = NetF(H.shape[1],self.parameters.d)
        Normalization = nn.BatchNorm1d(H.shape[1], affine=True)
        H = Normalization(H)
        Z = net5.forward(H)
        lr = 0.0001
        ...
        for epoch in range(250):
            ...
            if self.X.shape[0] == self.X.shape[1]:
                if (self.X == torch.eye(self.X.shape[1])).any() == False:  # 判断是否是单位阵
                    H = torch.cat((H, self.X), 1)
            else:
                H = torch.cat((H, self.X), 1)
            Normalization = nn.BatchNorm1d(H.shape[1], affine=True)
            H = Normalization(H)
            Z = net5.forward(H)
            print('第',self.num,'次运行算法中的第', epoch + 1, '波训练的损失函数值loss=', loss.detach().numpy())
        print('模型训练过程已结束！')
        file = open('acm-loss-sum(tau=2-beta=0.5-gamma=0.5).txt', 'w')
...
if __name__ == '__main__':#CPU version
    ...
    flag = int(input('Is graph an attribute graph? (1.YES 2.NO) 1 or 2:'))
    if flag == 1:
    ...
    parameters = Parameters(beta=0.5, gamma=0.5, fun='softmax', d=k_clusters, k=k_clusters, layers=[200,170,140,100],cat='CAT')# Set the parameters of SDGE 100
    num = 20
    JJ = []
    FMM = []
    FF1 = []
